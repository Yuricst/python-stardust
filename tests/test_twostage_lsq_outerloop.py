...
def test_twostage_outerloop_parallel():
    # physical parameters
    LU = 389703     # km
    TU = 382981     # sec
    MU = 1000.0     # kg
    ...
    # test outer loop with parallel Jacobian computation
    print(f"\nTesting outer loop in sparse Jacobian mode (parallel)...")
    np.random.seed(0)       # set flag since we are using random initial guess
    prob = stardust.FixedTimeTwoStageLeastSquares(     # need to re-initialize
        stardust.eom_stm_rotating_cr3bp,
        rv0,
...
if __name__=="__main__":
    ...
    test_twostage_outerloop_parallel()
    print("Done!")
    plt.show()
